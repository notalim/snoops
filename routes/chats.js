...
// TODO: GET /:uid/:acid - Getting all chats between all users
    ...
    try{
        ...
        let sorted = chatList.sort((a,b) => {
            ...
            if(a.messages.length === 0){
                ...
                return b.messages[b.messages.length-1].messageTime
                .localeCompare(a.messages[a.messages.length-1].messageTime)
            }
...
router.route("/:uid/:acid").post(async (req, res) => {
    ...
    try{
        const chat = await chatData.getChat(uid, acid);
        res.status(400).json({foundChat: chat, Message: "Chat already exist"});
    }catch(e){
        try{
